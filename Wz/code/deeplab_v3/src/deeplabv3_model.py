...
#-----------------------------------#
#   分支5
#   1x1AVGPooling+1x1Conv+BN+ReLU+上采样
#-----------------------------------#
...
class ASPP(nn.Module):
    def __init__(self, in_channels: int, atrous_rates: List[int], out_channels: int = 256) -> None:
        """
        in_channels:    输入通道
        atrous_rates:   中间3个分支的膨胀系数 [12, 24, 36]
        out_channels:   输出通道数
        """
        ...
        #-----------------------------------#
        #   拼接后的1x1Conv+BN+ReLU,调整通道数
        #   5*out_channels -> out_channels
        #-----------------------------------#
...
class DeepLabHead(nn.Sequential):
    def __init__(self, in_channels: int, num_classes: int) -> None:
        super(DeepLabHead, self).__init__(
            ASPP(in_channels, [12, 24, 36]),                # [12, 24, 36] 是中间3个分支的膨胀系数
            nn.Conv2d(256, 256, 3, padding=1, bias=False),  # 3x3Conv 特征融合
            nn.BatchNorm2d(256),
            nn.ReLU(),
            nn.Conv2d(256, num_classes, 1)                  # 1x1Conv 调整通道数
        )
